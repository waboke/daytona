...
func selectPrebuildPrompt(prebuilds []apiclient.PrebuildDTO, actionVerb string, choiceChan chan<- *apiclient.PrebuildDTO) {
	...
	for _, pb := range prebuilds {
		desc := fmt.Sprintf("Branch: %s (every %d commits)", pb.Branch, pb.CommitInterval)

		newItem := item[apiclient.PrebuildDTO]{title: pb.ProjectConfigName, desc: desc, choiceProperty: pb}
